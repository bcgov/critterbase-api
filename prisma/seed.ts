...
import {
  taxonMeasurementQualitativeData,
  taxonMeasurementQuantitativeData,
} from "./seed_scripts/seed_measurements"
...
async function main() {
  ...
  if (!rangiferTarandusUUID || !canisLupusUUID || !mammaliaUUID || !artioUUID) {
    throw Error(
      "Unable to grab the UUID for a required taxon " +
  ...
  await prisma.xref_taxon_measurement_qualitative.createMany({
    data: taxonMeasurementQualitativeData.map((q) => {
      return {
        taxon_id: q.taxon_id,
  ...
  for (const q of taxonMeasurementQualitativeData) {
    const uuid = qualitativeResults.find(
      (a) =>
        a.measurement_name === q.measurement_name && a.taxon_id === q.taxon_id
  ...
  await prisma.xref_taxon_measurement_quantitative.createMany({
    data: taxonMeasurementQuantitativeData.map((q) => {
      return {
        measurement_name: q.measurement_name,
        taxon_id: q.taxon_id,
