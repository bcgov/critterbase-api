...
import { MeasurementQualitativeIncludeSchema, MeasurementQuantitativeIncludeSchema, MeasurementVerificationSchema, QualitativeCreateSchema, QualitativeSchema, QuantitativeCreateSchema, QuantitativeSchema, QuantitativeUpdateSchema } from "./measurement.utils"
...
const PostVerifyMeasurements: ZodOpenApiOperationObject = {
  operationId: 'verifyMeasurements',
  summary: 'Verify measurements against taxon id.',
  tags: [TAG],
  requestBody: {
    content: {
      'application/json': {
        schema: MeasurementVerificationSchema,
      }
    }
  },
  responses: {
    '200': {
      ...
      content: {
        'application/json': {
          schema: z.object({
            verified: z.boolean(),
            invalid_measurements: z.object({
              qualitative_measurement_ids: zodID.array(),
              quantitative_measurement_ids: zodID.array()
            })
          })
        }
      }
...
export const measurementPaths = {
  ...
  [`${routes.measurements}/verify`]: {
    get: PostVerifyMeasurements
  },
