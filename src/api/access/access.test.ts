...
import { getTableDataTypes as _getTableDataTypes, loginUser as _loginUser } from './access.service';
...
describe('SERVICES', () => {
  describe('loginUser', () => {
    ...
    it('keycloak_uuid: login fails with invalid formatted keycloak_uuid', async () => {
      findFirst.mockResolvedValue(null);
      await expect(_loginUser({ keycloak_uuid: 'test' })).rejects.toThrowError(apiError);
      expect(prisma.user.findFirst).toHaveBeenCalledTimes(1);
    });
  });
  ...
  describe('ROUTERS', () => {
    ...
    describe('/api/signup', () => {
      it('should return status 200 with valid body', async () => {
        const res = await request.post('/api/signup').send({ user_identifier: 1 });
