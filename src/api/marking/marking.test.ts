...
const getBodyLocationByNameAndTaxonUUID = jest.spyOn(lookups, "getBodyLocationByNameAndTaxonUUID").mockImplementation();
  ...
  describe("SERVICES", () => {
    ...
    describe("appendEnglishMarkingsAsUUID()", () => {
      it("should append uuids based on english marking field names", async () => {
        ...
        const new_body = await  _appendEnglishMarkingsAsUUID(body, 'f91ae2d3-4f5b-4502-aceb-264351709695');
        expect.assertions(5);
  ...
  describe("ROUTERS", () => {
    describe("GET /api/markings", () => {
      it("returns status 200", async () => {
        ...
        expect(getAllMarkings.mock.results[0].value[0].marking_id).toBe(MARKING_ID);
      });
    });
    ...
    describe("GET /api/markings/critter/:id", () => {
      ...
      it("returns status 200", async () => {
        ...
        const res = await request.get(
          `/api/markings/critter/${CRITTER_ID}`
        );
        expect.assertions(3);
        expect(res.status).toBe(200);
        expect(getMarkingsByCritterId.mock.calls.length).toBe(1);
        expect(getMarkingsByCritterId.mock.results[0].value[0].marking_id).toBe(MARKING_ID);
      });
    });
  });
});