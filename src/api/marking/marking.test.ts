  ...
  describe('ROUTERS', () => {
    ...
    describe('POST /api/markings/create', () => {
      ...
      it('strips invalid fields from data', async () => {
        const res = await request.post('/api/markings/create').send({ ...MARKING, invalidField: 'qwerty123' });
        expect.assertions(3);
        expect(res.status).toBe(201);
        expect(createMarking.mock.calls.length).toBe(1);
    ...
    describe('PATCH /api/markings/:id', () => {
      it('returns status 404 when id does not exist', async () => {
        ...
        const res = await request.patch(`/api/markings/${randomUUID()}`).send(MARKING);
        expect.assertions(1);
        expect(res.status).toBe(404);
      });

      it('returns status 200', async () => {
        const res = await request.patch(`/api/markings/${MARKING_ID}`).send(MARKING);
        expect.assertions(1);
        expect(res.status).toBe(200);
      });
