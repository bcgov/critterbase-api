...
import { FormattedLocation } from "../location/location.utils";
...
const formatCapture = (capture: CaptureIncludeType) => {
  ...
  if (capture.location_capture_capture_location_idTolocation) {
    obj = {
      ...capture,
      capture_location: formatLocation(
        capture.location_capture_capture_location_idTolocation
      ),
    };
  }
  if (capture.location_capture_release_location_idTolocation) {
    obj = {
      ...obj,
      release_location: formatLocation(
        capture.location_capture_release_location_idTolocation
      ),
    };
  }
