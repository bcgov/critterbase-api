...
const CritterResponseSchema = ResponseSchema
.transform((val) => {
    const {
      mortality,
      capture,
    ...
    return {
      ...
      mortality: mortality.map(a => stripExtraFields(MortalityResponseSchema.parse(a))),
      capture: capture.map(a => stripExtraFields(CaptureResponseSchema.parse(a))),
      marking: marking.map(a => stripExtraFields(markingResponseSchema.parse(a)) ),
      measurement: {
        qualitative: measurement_qualitative.map(a => stripExtraFields(QualitativeResponseSchema.parse(a))),
        quantitative: measurement_quantitative.map(a => stripExtraFields(QuantitativeResponseSchema.parse(a)))
      }
    }
  });
  ...
  const stripExtraFields = <T extends critterInterface>(obj: T): 
