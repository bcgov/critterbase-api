import { capture, critter, lk_taxon, marking, measurement_qualitative, measurement_quantitative, mortality } from "@prisma/client";
...
import { CritterDefaultResponseSchema, CritterDetailedResponseSchema, CritterSchema, CritterUpdateSchema } from "./critter.utils";
  ...
  describe("UTILS", () => {
    describe("CritterDetailedResponseSchema", () => {
      it("should correctly parse the object", async () => {
        const parsed = CritterDetailedResponseSchema.parse(DETAILEDFORMAT_CRITTER);
        expect.assertions(5);
      ...
      it("should handle the case where there is only taxon_name_latin available", async () => {
        ...
        const parsed = CritterDetailedResponseSchema.parse({...rest, lk_taxon: { taxon_name_latin: 'Ranger tarandus'}});
      ...
      it("should handle the case where there is no mortality", async () => {
        ...
        const parsed = CritterDetailedResponseSchema.parse({...rest, mortality: []});
        expect(parsed.mortality_timestamp).toBeNull();
      });
    });
    describe("CritterDefaultResponseSchema", () => {
      it("should correctly parse the object", async () => {
        const parsed = CritterDefaultResponseSchema.parse(DEFAULTFORMAT_CRITTER);
        expect.assertions(3);
        expect(parsed.taxon).toBeDefined();
      ...
      it("should handle the case where only taxon_name_latin is available", async () => {
        ...
        const parsed = CritterDefaultResponseSchema.parse({...rest, lk_taxon: { taxon_name_latin: 'Ranger tarandus'}});
        expect(parsed.taxon).toBe('Ranger tarandus');
      });
      it("should handle the case where there is no mortality", async () => {
        const {mortality, ...rest} = DEFAULTFORMAT_CRITTER;
        const parsed = CritterDefaultResponseSchema.parse({...rest, mortality: []});
        expect(parsed.mortality_timestamp).toBeNull();
      });
    })
  })
  describe("SERVICES", () => {
    describe("createCritter", () => {
      it("creates a critter", async () => {
        ...
        expect(CritterDefaultResponseSchema.safeParse(returnedCritter).success).toBe(true);
      });
      it("creates a critter, detailed foramt", async () => {
        create.mockResolvedValue(DETAILEDFORMAT_CRITTER);
        const returnedCritter = await _createCritter(CRITTER, QueryFormats.detailed);
        expect.assertions(2);
        expect(prisma.critter.create).toHaveBeenCalled();
        expect(CritterDetailedResponseSchema.safeParse(returnedCritter).success).toBe(true);
      });
      it("creates a critter", async () => {
        create.mockResolvedValue(CRITTER);
        const returnedCritter = await _createCritter(CRITTER, QueryFormats.asSelect);
        expect.assertions(2);
        expect(prisma.critter.create).toHaveBeenCalled();
    ...
    describe("getAllCritters()", () => {
      it("returns many critters", async () => {
        ...
        expect(CritterDefaultResponseSchema.safeParse(returnedCritter[0]).success).toBe(true);
      });
      it("test the detailed format case", async () => {
        ...
        expect(returnedCritter).toBeInstanceOf(Array);
        expect(returnedCritter.length).toBe(1);
        expect(CritterDetailedResponseSchema.safeParse(returnedCritter[0]).success).toBe(true);
      });
    ...
    describe("getCritterById()", () => {
      ...
      it("returnes a critter by ID, detailed format", async () => {
        ...
        const returnedCritter = await _getCritterById(CRITTER_ID, QueryFormats.detailed);
        expect.assertions(2);
        expect(CritterDetailedResponseSchema.safeParse(returnedCritter).success).toBe(true);
        expect(prisma.critter.findUniqueOrThrow).toHaveBeenCalled();
      });
      it("returnes a critter by ID, no prisma include", async () => {
        ...
        const returnedCritter = await _getCritterById(CRITTER_ID, QueryFormats.asSelect);
        expect.assertions(2);
        expect(returnedCritter['lk_taxon']).toBeUndefined();
        expect(prisma.critter.findUniqueOrThrow).toHaveBeenCalled();
      });
    ...
    describe("getCritterByWlhId(wlh_id: string)", () => {
      ...
      it("returns a critter by wlh_id, detailed format", async () => {
        findMany.mockResolvedValue([DETAILEDFORMAT_CRITTER]);
        const returnedCritter = await _getCritterByWlhId(WLH_ID, QueryFormats.detailed);
        expect.assertions(2);
        expect(CritterDetailedResponseSchema.safeParse(returnedCritter[0]).success).toBe(true);
        expect(prisma.critter.findMany).toHaveBeenCalled();
      });
      it("returns a critter by wlh_id", async () => {
        findMany.mockResolvedValue([CRITTER]);
        const returnedCritter = await _getCritterByWlhId(WLH_ID, QueryFormats.asSelect);
        expect.assertions(2);
        expect(returnedCritter[0]['lk_taxon']).toBeUndefined();
        expect(prisma.critter.findMany).toHaveBeenCalled();
      });
    ...
    describe("updateCritter()", () => {
      ...
      it("updates a critter with the body provided", async () => {
        ...
        const returnedCritter = await _updateCritter(CRITTER_ID, CRITTER, QueryFormats.detailed);
        expect.assertions(2);
        expect(prisma.critter.update).toHaveBeenCalled();
        expect(CritterDetailedResponseSchema.safeParse(returnedCritter).success).toBe(true);
      });
      it("updates a critter with the body provided", async () => {
        ...
        const returnedCritter = await _updateCritter(CRITTER_ID, CRITTER, QueryFormats.asSelect);
        expect.assertions(2);
        expect(prisma.critter.update).toHaveBeenCalled();
        expect(returnedCritter['lk_taxon']).toBeUndefined();
      })
    ...
    describe("deleteCritter()", () => {
      ...
      it("should delete a critter, detailed format", async () => {
        ...
        const returnedCritter = await _deleteCritter(CRITTER_ID, QueryFormats.detailed);
        expect.assertions(2);
        expect(prisma.critter.delete).toHaveBeenCalled();
        expect(CritterDetailedResponseSchema.safeParse(returnedCritter).success).toBe(true);
      })
      it("should delete a critter, no prisma include", async () => {
        ...
        const returnedCritter = await _deleteCritter(CRITTER_ID, QueryFormats.asSelect);
        expect.assertions(2);
        expect(prisma.critter.delete).toHaveBeenCalled();
        expect(returnedCritter['lk_taxon']).toBeUndefined();
      })
    ...
    describe("getMultipleCrittersByIds()", () => {
      it("should return critters matching the given ids", async () => {
        findMany.mockResolvedValue([DEFAULTFORMAT_CRITTER, {...DEFAULTFORMAT_CRITTER, ...OTHER_CRITTER}]);
      ...
      it("should return critters matching the given ids", async () => {
        findMany.mockResolvedValue([DETAILEDFORMAT_CRITTER]);
        const returnedCritters = await _getMultipleCrittersByIds({critter_ids: [CRITTER_ID]}, QueryFormats.detailed);
        expect.assertions(4);
        expect(prisma.critter.findMany).toHaveBeenCalled();
        expect(returnedCritters).toBeInstanceOf(Array);
      ...
      it("should handle the case where no prisma include is available", async () => {
        ...
        const returnedCritters = await _getMultipleCrittersByIds({critter_ids: [CRITTER_ID, OTHER_CRITTER_ID]}, QueryFormats.asSelect);
        expect.assertions(3);
        expect(prisma.critter.findMany).toHaveBeenCalled();
        expect(returnedCritters).toBeInstanceOf(Array);
    ...
    describe("getSimilarCritters()", () => {
      it("should return critters by their wlh_id, with various formatting", async () => {
        ...
        expect.assertions(3);
        expect(CritterDefaultResponseSchema.safeParse(returnedCritters[0]).success).toBe(true);

        findMany.mockResolvedValue([DETAILEDFORMAT_CRITTER]);
        const returnedCritters2 = await _getSimilarCritters({ critter: { wlh_id: WLH_ID }}, QueryFormats.detailed);
        expect(CritterDetailedResponseSchema.safeParse(returnedCritters2[0]).success).toBe(true);

        findMany.mockResolvedValue([CRITTER]);
        const returnedCritters3 = await _getSimilarCritters({ critter: { wlh_id: WLH_ID }}, QueryFormats.asSelect);
        expect(returnedCritters3[0]['lk_taxon']).toBeUndefined();
      })
      it("should return critters by their wlh_id, taxon", async () => {
        findMany.mockResolvedValue([DEFAULTFORMAT_CRITTER]);
        const returnedCritters = await _getSimilarCritters({ critter: { wlh_id: WLH_ID, taxon_name_common: 'Rangifer tarandus', taxon_name_latin: 'Caribou' }});
        expect.assertions(9);
        expect(prisma.critter.findMany).toHaveBeenCalledTimes(2);
        expect(returnedCritters).toBeInstanceOf(Array);
        expect(returnedCritters.length).toBe(1);
        const returnedCritters2 = await _getSimilarCritters({ critter: { wlh_id: WLH_ID, taxon_name_common: 'Rangifer tarandus' }});;
        ...
        const returnedCritters3 = await _getSimilarCritters({ critter: { wlh_id: WLH_ID, taxon_name_latin: 'Caribou' }});;
      ...
      it("should return critters based off marking and critter detail", async () => {
        ...
        const returnedCritters = await _getSimilarCritters({ critter: {wlh_id: WLH_ID}, markings: [{primary_colour: 'Red'}]});
        expect.assertions(4);
      ...
      it("should return critters by their markings", async () => {
        expect.assertions(3);
        markingFindMany.mockResolvedValue([MARKING]);
        const returnedCritters1 = await _getSimilarCritters({ markings: [{marking_type: 'Ear Tag', primary_colour: 'Red', secondary_colour: 'Red', body_location: 'Left Ear', identifier: '11'}]});
      ...
      it("should return critters by capture data", async () => {
        ...
        const returnedCritters1 = await _getSimilarCritters({ captures: [{ capture_timestamp: new Date(), release_timestamp: new Date() }]});
        findMany.mockResolvedValueOnce([]);
      ...
      it("should return critters by mortality", async () => {
        ...
        const returnedCritters1 = await _getSimilarCritters({ mortality: {mortality_timestamp: new Date()}});
        findMany.mockResolvedValueOnce([]);
    ...
    describe("POST /api/critters/filter", () => {
      it("should return status 200", async () => {
        ...
        const body = {
          ...
          collection_units: {body: ['1af85263-6a7e-4b76-8ca6-118fd3c43f50'], negate: false}
        }
        ...
        const body2 = {
          ...
          collection_units: {body: ['1af85263-6a7e-4b76-8ca6-118fd3c43f50'], negate: true}
        };
      ...
      it("should return status 200, test taxon_ids only case", async () => {
        const body = {
          taxon_ids: {body: ['1af85263-6a7e-4b76-8ca6-118fd3c43f50'], negate: true}
        };
        const res = await request.post("/api/critters/filter").send(body);
        expect.assertions(2);
        expect(res.status).toBe(200);
