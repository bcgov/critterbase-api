...
const request = supertest(makeApp({ critterService: mockCritterService } as any));
...
describe('Critter Router', () => {
  describe('GET critter/ - get all critters or critters with matching wlh_id', () => {
    it('should respond with status 200 and return response', async () => {
      ...
      expect(mockCritterService.getAllCrittersOrCrittersWithWlhId).toHaveBeenCalled();
    ...
    it('should respond 200 and pass wlh_id to service function', async () => {
      ...
      expect(mockCritterService.getAllCrittersOrCrittersWithWlhId.mock.calls[0][0]).toBe('wlhid');
      expect(res.status).toBe(200);
      expect(res.body).toBe(true);
    });
  });
  ...
  describe("POST critter/ - get all critters with matching critter_id's", () => {
    it('should respond with status 200 and return response', async () => {
      ...
      const res = await request.post('/api/critters').send({ critter_ids: [id] });

      expect(res.status).toBe(200);
      expect(res.body).toBe(true);
      expect(mockCritterService.getMultipleCrittersByIds.mock.calls[0][0]).toStrictEqual([id]);
    });
    ...
    describe('GET critter/:id - get a critter by id', () => {
      it('should respond with status 200 and return response', async () => {
        ...
        const res = await request.get(`/api/critters/${id}`).query({ format: 'asSelect' });

        expect(mockCritterService.getCritterById.mock.calls[0][0]).toStrictEqual(id);
        expect(mockCritterService.getCritterById.mock.calls[0][1]).toStrictEqual('asSelect');
        expect(res.status).toBe(200);
        expect(res.body).toBe(true);
      });
    });
    ...
    describe('PATCH critter/:id - update a critter by id', () => {
      it('should respond with status 201 and return response', async () => {
        ...
        expect(mockCritterService.updateCritter.mock.calls[0][0]).toStrictEqual(id);
        expect(res.status).toBe(201);
        expect(res.body).toBe(true);
      });
    });
