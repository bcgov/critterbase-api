...
const colourFormats: FormatParse = {
  asSelect: {
    schema: ResponseSchema.transform((val) => toSelect<lk_colour>(val, 'colour_id', 'colour'))
  }
};

const regionEnvFormats: FormatParse = {
  asSelect: {
    schema: ResponseSchema.transform((val) => toSelect<lk_region_env>(val, 'region_env_id', 'region_env_name'))
  }
};

const regionNrFormats: FormatParse = {
  asSelect: {
    schema: ResponseSchema.transform((val) => toSelect<lk_region_nr>(val, 'region_nr_id', 'region_nr_name'))
  }
};

const wmuFormats: FormatParse = {
  asSelect: {
    schema: ResponseSchema.transform((val) => toSelect<lk_wildlife_management_unit>(val, 'wmu_id', 'wmu_name'))
  }
};
...
const markingMaterialsFormats: FormatParse = {
  asSelect: {
    schema: ResponseSchema.transform((val) => toSelect<lk_marking_material>(val, 'marking_material_id', 'material'))
  }
};

const markingTypesFormats: FormatParse = {
  asSelect: {
    schema: ResponseSchema.transform((val) => toSelect<lk_marking_type>(val, 'marking_type_id', 'name'))
  }
};
...
const collectionUnitCategoriesFormats: FormatParse = {
  asSelect: {
    schema: ResponseSchema.transform((val) =>
      toSelect<lk_collection_category>(val, 'collection_category_id', 'category_name')
    )
  }
};
