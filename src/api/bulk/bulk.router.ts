...
bulkRouter.post(
  ...
  catchErrors(async (req: Request, res: Response) => {
    ...
    const crittersAppend = critters
      ? await Promise.all(
          critters.map(async (c: CritterCreate) => {
    ...
    const markingsAppend = markings
      ? await Promise.all(
          markings.map(async (m: MarkingCreateInput) =>
            MarkingCreateBodySchema.parseAsync(m)
          )
        )
      : [];

    const parsedCaptures = captures
      ? captures.map((c: CaptureCreate) => CaptureCreateSchema.parse(c))
      : [];

    const parsedMortalities = mortalities
      ? await Promise.all(
          mortalities.map(async (m: MortalityCreate) => {
