...
const getCollectionUnitsFromCategory = async (category_name: string, taxon_name_common: string | undefined, taxon_name_latin: string | undefined) => {
    const taxon_categories = await prisma.xref_taxon_collection_category.findFirstOrThrow({
      where: {
        lk_taxon: { 
            taxon_name_common: taxon_name_common ? { equals: taxon_name_common, mode: 'insensitive' } : undefined,
            taxon_name_latin: taxon_name_latin ?  { equals: taxon_name_latin, mode: 'insensitive' } : undefined 
        },
...
export { getCollectionUnitsFromCategory }